...
def function(i):
    ...
    if i <1:
       ...
       x= (lamb*(i)**lamb)/((i+.1)**(lamb+1))

    return x
#Whether you want to do dynamic or not, if it is set to True then it overrides 
#the learning rate used in Sklearn's parameters
Dynamic = True    
#Paramters to use in the tree
params = {'n_estimators': n_esti, 'max_depth': 1, 'min_samples_split': 2,
          'learning_rate': 0.01}


'''The results we are interested in is an array of MSEs found in the "Sklearn_loss" variable'''
# Load data from Skleasrn Dataset
# #############################################################################
        ...
        for k in range(loss.K):
            ...
            if Dynamic is True:
                ...
                if i == 0:
                    if X_csr is not None:
                        loss.update_terminal_regions(tree.tree_, X_csr, y, residual, y_pred,
                                                     sample_weight, sample_mask, self.learning_rate, k=k)
                       #############################################################################################################################################   ###########################################                          
                    else:
                        loss.update_terminal_regions(tree.tree_, X, y, residual, y_pred,
                                                     sample_weight, sample_mask,
...
#Here is the tree    
tree = GradientBoostingRegressor(**params)
tree.fit(xi,yi)
sklearn_predictions = tree.predict(X_test)
sklearn_loss = np.zeros((n_esti,), dtype=np.float64)
for i, sklearn_predictions in enumerate(tree.staged_predict(X_test)):
        # clf.loss_ assumes that y_test[i] in {0, 1}
        sklearn_loss[i] = tree.loss_(y_test,sklearn_predictions)
